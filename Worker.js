...
// Strategie:
//  1. Link  → probeer eerst gratis schema.org/JSON-LD parsing, dan (als
//             ANTHROPIC_API_KEY gezet is) automatisch naar Engels vertalen.
//  2. Link  → geen schema.org-data gevonden? Val terug op Claude Haiku 4.5.
//  3. Foto  → altijd Claude Haiku 4.5 (vision).
//  Elke toevoeging vereist een geldige toegangscode (ACCESS_CODE secret).
//
// Secrets (`wrangler secret put <naam>`, of via Cloudflare dashboard):
//   ACCESS_CODE          → zelf verzonnen code, deel je alleen met wie mag toevoegen
//   ANTHROPIC_API_KEY    → je Claude API key (optioneel; zonder key geen foto's/vertaling)
...
function b64ToUtf8(b64) { const bytes = Uint8Array.from(atob(b64), (c) => c.charCodeAt(0)); return new TextDecoder().decode(bytes); }
function slugify(text) {
  return (text || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    .replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 60) || `recept-${Date.now()}`;
}
...
function schemaToOurFormat(schemaRecipe, sourceUrl) {
  ...
  const nl = { title, category: CATEGORY_LABELS[categoryId].nl, ingredient_groups: [{ name: "Ingrediënten", items: ingredients }], steps, notes: [] };
  return {
    ...
    nl, en: { ...nl, category: CATEGORY_LABELS[categoryId].en }, // wordt hieronder evt. echt vertaald
    source: new URL(sourceUrl).hostname.replace(/^www\./, ""), source_url: sourceUrl, page: null,
  };
}

// ── Claude Haiku 4.5: extractie (foto's, tekst-fallback) én vertaling ─────
const RECIPE_TOOL = {
  name: "save_recipe",
  ...
  input_schema: {
    type: "object",
    properties: {
      id: { type: "string" }, category_id: { type: "string", enum: Object.keys(CATEGORY_LABELS) },
      servings: { type: ["string", "null"] }, prep_time_minutes: { type: "integer" },
      passive_time_minutes: { type: "integer" }, cook_time_minutes: { type: "integer" },
      total_time_minutes: { type: "integer" }, time_estimated: { type: "boolean" },
      nl: { type: "object", properties: { title: { type: "string" }, ingredient_groups: { type: "array", items: { type: "object", properties: { name: { type: "string" }, items: { type: "array", items: { type: "string" } } }, required: ["name", "items"] } }, steps: { type: "array", items: { type: "string" } }, notes: { type: "array", items: { type: "string" } } }, required: ["title", "ingredient_groups", "steps", "notes"] },
      en: { type: "object", properties: { title: { type: "string" }, ingredient_groups: { type: "array", items: { type: "object", properties: { name: { type: "string" }, items: { type: "array", items: { type: "string" } } }, required: ["name", "items"] } }, steps: { type: "array", items: { type: "string" } }, notes: { type: "array", items: { type: "string" } } }, required: ["title", "ingredient_groups", "steps", "notes"] },
...
async function callClaude(env, { system, messages, tool }) {
  const r = await fetch("https://api.anthropic.com/v1/messages", {
    method: "POST",
    headers: { "x-api-key": env.ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01", "Content-Type": "application/json" },
    body: JSON.stringify({ model: CLAUDE_MODEL, max_tokens: 4096, system, messages, tools: [tool], tool_choice: { type: "tool", name: tool.name } }),
  });
...
async function extractWithClaude(env, contentBlocks) {
  const recipe = await callClaude(env, { system: systemPrompt(), messages: [{ role: "user", content: contentBlocks }], tool: RECIPE_TOOL });
...
const TRANSLATE_TOOL = {
  ...
  description: "Sla de Engelse vertaling van het recept op.",
  input_schema: {
    type: "object",
    properties: {
      title: { type: "string" },
...
async function translateToEnglish(env, nl) {
  const translated = await callClaude(env, {
    system: "Vertaal dit Nederlandse recept naar natuurlijk Engels via de tool save_translation. Behoud exact hetzelfde aantal ingrediëntgroepen, items per groep en stappen, in dezelfde volgorde — alleen de tekst wordt vertaald.",
    messages: [{ role: "user", content: JSON.stringify({ title: nl.title, ingredient_groups: nl.ingredient_groups, steps: nl.steps, notes: nl.notes }) }],
    tool: TRANSLATE_TOOL,
  });
  return translated;
}
...
async function commitRecipe(env, recipe) {
  const headers = { "Authorization": `Bearer ${env.GITHUB_TOKEN}`, "Accept": "application/vnd.github+json", "X-GitHub-Api-Version": "2022-11-28", "User-Agent": "recepten-api-worker" };
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      let recipe = null, method = "", code = "";

      if (contentType.includes("application/json")) {
        const body = await request.json();
        code = body.code || "";
        if (!env.ACCESS_CODE || code !== env.ACCESS_CODE) return reply({ error: "Onjuiste of ontbrekende toegangscode" }, 401, origin);
        ...
        if (recipe) {
          ...
          if (env.ANTHROPIC_API_KEY) {
            try { recipe.en = { ...(await translateToEnglish(env, recipe.nl)), category: recipe.en.category }; method += " + AI-vertaling"; }
            catch { /* vertaling mislukt: EN blijft kopie van NL, verder geen probleem */ }
          }
        } else {
        ...
        recipe.source = "Kookboekfoto"; recipe.source_url = null; recipe.page = null;
